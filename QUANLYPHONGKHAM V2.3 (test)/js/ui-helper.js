 ...
 * CẬP NHẬT: Sửa lỗi bàn phím số đóng sớm khi nhập một chữ số.
 */
...
// Hàm tạo container chứa thông báo (Chạy tự động khi cần)
 ...
 * @param {string} type - Loại: 'success' (Xanh), 'error' (Đỏ), 'warning' (Vàng), 'info' (Mặc định)
 */
window.showToast = function(message, type = 'info') {
    ...
    // Hiệu ứng trượt vào (dùng setTimeout nhỏ để CSS transition bắt được)
    ...
    setTimeout(() => {
        ...
        // Xóa hoàn toàn khỏi DOM sau khi animation kết thúc (300ms)
        setTimeout(() => {
...
// --- 6. GHI ĐÈ ALERT MẶC ĐỊNH ---
// Biến dòng lệnh này thành công cụ nâng cấp trải nghiệm tức thì
window.originalAlert = window.alert;
window.alert = function(message) {
    // Chuyển tất cả alert thường thành Toast Warning (hoặc Info)
    window.showToast(message, 'warning');
    console.log("System Alert:", message); // Vẫn log ra console để debug nếu cần
};

// Hàm tiện ích gọi nhanh cho các trường hợp khác
window.notifySuccess = (msg) => window.showToast(msg, 'success');
window.notifyError = (msg) => window.showToast(msg, 'error');
// --- 7. CHỨC NĂNG FULL SCREEN (CHO IPAD SHORTCUT) ---

window.toggleFullScreen = function() {
    var doc = window.document;
    var docEl = doc.documentElement;

    // Kiểm tra các hàm API của từng trình duyệt (Chrome, Safari, Firefox, IE)
    var requestFullScreen = docEl.requestFullscreen || docEl.mozRequestFullScreen || docEl.webkitRequestFullscreen || docEl.msRequestFullscreen;
    var cancelFullScreen = doc.exitFullscreen || doc.mozCancelFullScreen || doc.webkitExitFullscreen || doc.msExitFullscreen;

    // Nếu chưa Full Screen -> Mở Full
    if(!doc.fullscreenElement && !doc.mozFullScreenElement && !doc.webkitFullscreenElement && !doc.msFullscreenElement) {
        if(requestFullScreen) {
            ...
            alert("Trình duyệt này không hỗ trợ Full Screen tự động.");
        }
    } 
    // Nếu đang Full -> Thoát
    else {
