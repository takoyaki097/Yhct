/**
 * FILE: knowledge/herbs-data.js
 * CHỨC NĂNG: Cơ sở dữ liệu Dược liệu & Phân nhóm thuốc
 * CƠ CHẾ: Tự động nối tiếp vào mảng window.knowledge.herbsDB
 */

window.knowledge = window.knowledge || {};
window.knowledge.herbsDB = window.knowledge.herbsDB || [];
...
(function() {
    const herbsData = [
        ...
        // MẸO: Thạc sĩ có thể thêm thuốc mới vào ngay dưới dòng này theo mẫu:
        // { name: 'Tên Thuốc Mới', category: 'Nhóm Mới', tags: ['tác dụng 1', 'tác dụng 2'] },
    ];

    // Nối dữ liệu vào mảng chính
    window.knowledge.herbsDB = window.knowledge.herbsDB.concat(herbsData);

    // Danh sách phân loại (Dùng cho Sidebar)
    window.knowledge.herbCategories = [
        'Giải Biểu', 'Bổ Dưỡng', 'Ôn Lý', 'Thanh Nhiệt', 'Trừ Đàm', 
        'Lý Khí', 'Hoạt Huyết', 'An Thần', 'Lợi Thủy'
    ];
})();
